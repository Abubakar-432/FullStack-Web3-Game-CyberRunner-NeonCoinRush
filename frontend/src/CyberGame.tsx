...
interface GameProps {
  ...
  hasShield: boolean;
  hasNFT: boolean;
  // optional name/label for the hoverboard
  hoverboardName?: string;
  // we no longer control audio here; App handles music
  musicTrack?: "track1" | "track2";
}

const CyberGame = ({ onGameOver, hasMagnet, hasShield, hasNFT, hoverboardName }: GameProps) => {
  ...
  // Label to show in HUD
  const hoverboardLabel =
    hoverboardName && hoverboardName.trim().length > 0
  ...
  useEffect(() => {
    magnetRef.current = hasMagnet;
  }, [hasMagnet]);

  useEffect(() => {
    nftRef.current = hasNFT;
  }, [hasNFT]);
  ...
  const SINK_OFFSET = 8.5;
  const OBSTACLE_WIDTH = 95;
  const OBSTACLE_HEIGHT = 155;
  const HOVER_OFFSET = 0;
  const OBSTACLE_Y_OFFSET = 45;
  ...
  useEffect(() => {
    ...
    const spawnEntity = () => {
        ...
        if (!lastObs || canvas.width - lastObs.x > 450) {
          ...
          if (type === "wall") {
            gameState.obstacles.push({
              x: canvas.width + 50,
              y: floorY - OBSTACLE_HEIGHT + OBSTACLE_Y_OFFSET,
              w: OBSTACLE_WIDTH,
              h: OBSTACLE_HEIGHT,
              type: "wall",
              passed: false,
      ...
      if (Math.random() < 0.04) {
        const lastObs = gameState.obstacles[gameState.obstacles.length - 1];
        const floorYCoins = canvas.height - FLOOR_OFFSET;
        let coinX = canvas.width + 100;
        let coinY = floorYCoins - 150 + Math.random() * 50;
        ...
        for (const obs of gameState.obstacles) {
          if (obs.type === "platform") {
            if (playerX + playerWidth > obs.x && playerX < obs.x + obs.w) {
              ...
              if (
                gameState.playerY >= obs.y - gameState.hoverOffset &&
                gameState.playerY <= obs.y + tolerance - gameState.hoverOffset
              ) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {shieldVisual && <div style={{ fontSize: "1rem", color: "#00fff5" }}>🛡️ SHIELD ON</div>}

        {magnetRef.current && <div style={{ fontSize: "1rem", color: "#f9bc60" }}>🧲 MAGNET ON</div>}

        {nftRef.current && (
          <div style={{ fontSize: "1rem", color: "#ff00ff" }}>🛹 {hoverboardLabel}</div>
        )}
      </div>
